...
let _util = {
    ...
    getPicInfo:function (options) {
        let src = options.src || '';
        let fastCallback = options.fastCallback || new Function;
        let loadedCallback = options.loadedCallback;
        ...
        let rollpolling = function () {
            if(params.isError || params.width > 0 || params.height > 0){
                clearTimeout(timer);
...
class VueViewLoad {
     ...
     */
    constructor (options) {
        this.emptyPic = 'http://img.zcool.cn/community/0161f656b0663e6ac7256cb052d31a.gif';
    ...
    inView(ele){
        ...
        let parentRect = this.container == window ? {left:0,top:0} : this.container.getBoundingClientRect();
        let viewWidth = this.container == window ? window.innerWidth : this.container.clientWidth;
        let viewHeight = this.container == window ? window.innerHeight : this.container.clientHeight;
    ...
    render(){
        ...
        for (let i = 0; i < this.selector.length; i++) {
            ...
            let eleType = item.ele.nodeName.toLowerCase();
                    ...
                    _util.getPicInfo({
                        ...
                        loadedCallback: (options) => {
                            ...
                            setTimeout(()=>{
                                ...
                                item.ele.style.transition = 'all 1s linear'
                            },50)
                        }
                    });
