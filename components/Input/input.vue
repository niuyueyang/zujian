  ...
  methods: {
    ...
    inputEvent (e) {
      this.currValue = e.target.value
        this.$emit('on-input', this.value || this.value === '' ? this.currValue : e)
        this.dispatch('hFormItem', 'input', this.currValue)
    },
    blurEvent(){
      this.dispatch('hFormItem', 'blur', this.currValue)
    },
    changeEvent(){
      this.dispatch('hFormItem', 'change', this.currValue)
    }
  },
  watch: {
    value (value) {
